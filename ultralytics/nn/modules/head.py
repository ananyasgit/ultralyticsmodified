...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)  # New 4th channel conv
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()

    def forward(self, x):
        z = []  # inference output
        for i in range(self.nl):
            if not self.training:  # if not training use cached shape
                if self.dynamic:
                    x[i] = F.interpolate(x[i], size=self.shape[i], mode='nearest')
            x[i] = self.cv2[i](x[i])  # output conv
            z.append(x[i].clone())  # inference output
            if self.export:
                continue  # export mode only outputs 2nd stage
            x[i] = self.cv3[i](x[i])  # output conv
            z.append(x[i].clone())  # inference output
            x[i] = self.cv4[i](x[i])  # output conv for 4th channel
            z.append(x[i].clone())  # inference output
        if self.training:
            return x
        elif self.export:
    ...
    for a, b, c, s in zip(m.cv2, m.cv3, m.cv4, m.stride):  # from
        ...
        c[-1].bias.data[:m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls for 4th channel
